...
namespace objl
{
	...
	namespace algorithm
	{
		...
		inline std::string tail(const std::string &in)
		{
			...
			{
			   std::string ret = in.substr(tail_start, tail_end - tail_start + 1);
			  ret.erase(std::remove(ret.begin(), ret.end(), '\r'), ret.end());
			  return ret;
			}
			else if (tail_start != std::string::npos)
			{
			  std::string ret = in.substr(tail_start);
			  ret.erase(std::remove(ret.begin(), ret.end(), '\r'), ret.end());
			  return ret;
			}
			return "";
	...
	public:
		...
		bool LoadFile(std::string Path)
		{
			...
			std::cout << "LoadedMeshes size = " << LoadedMeshes.size() << " MeshMatNames size = " << MeshMatNames.size() << std::endl;
			// Set Materials for each Mesh
			for (int i = 0; i < MeshMatNames.size(); i++)
			{
				std::string matname = MeshMatNames[i];
				std::cout << "Matname " << matname << " i = " << i << std::endl;
